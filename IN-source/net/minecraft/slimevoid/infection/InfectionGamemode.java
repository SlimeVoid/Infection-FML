...
import net.minecraft.slimevoid.Gamemode;
...
public class InfectionGamemode extends Gamemode{
	...
	public boolean infect(int x, int y, int z, int infector, int infected, int availableAir) {
		...
		if (world.getTopSolidOrLiquidBlock(x, z) == y + 1) {
			for(int i = x - 1; i <= x + 1; i++) {
				for(int k = z - 1; k <= z + 1; k++) {
					world.setBlockWithNotify(i, 128, k, mod_Infection.INFECTED_SKY.blockID);
				}
			}
			world.setBlockWithNotify(x + 2, 128, z, mod_Infection.INFECTED_SKY.blockID);
			world.setBlockWithNotify(x - 2, 128, z, mod_Infection.INFECTED_SKY.blockID);
			world.setBlockWithNotify(x, 128, z + 2, mod_Infection.INFECTED_SKY.blockID);
			world.setBlockWithNotify(x, 128, z - 2, mod_Infection.INFECTED_SKY.blockID);
		}
...
//			entity.setDead(); 
	...
	public boolean canPlayerEdit(EntityPlayer player, int x, int y, int z, int id, boolean mine) {
		...
		if(!mine) {
			...
			if(id == mod_Infection.SPAWN.blockID) {
			...
			if(id == Block.leaves.blockID || id == mod_Infection.INFECTED_LEAVES.blockID) {
				return true;
			}
		}
		if(spawnBlock == null) {
